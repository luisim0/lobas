...
function onServerError(stack,msg){
	if(stack.tries == 3){
		...
		if(confirm("Ocurrió un problema con los servidores de AS Find. ¿Desea intentarlo de nuevo?")){
...
function is_session_active(){
	if(window.location.href.indexOf(login_con_url) != -1){
		...
		chrome.extension.sendMessage(jsoned,function(response){
			if(response.answer == 'Error'){
				alert("Los servidores de AS Find están temporalmente fuera de servicio. Por favor intente más tarde.");
				return false;
			}else{
				...
				if(yesno == "0"){//There's no session -- instruct to open session
					if(window.location.href.indexOf(login_con_url) == 0){
						jsoned = JSON.parse('{"action":"prompt_message","title":"Inicie sesión en AS Find","msg":"Inicie sesión en AS Find usando el ícono en la barra de navegación."}');
						chrome.extension.sendMessage(jsoned);
					}
					return false;
